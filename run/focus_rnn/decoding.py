...
from index2label import index2label as label2string
np.random.seed(3306)
...
def main(_):
    ...
    checkpoint_dir = root_path + '/all_in_one/demo/exported_models/' + mission + '/' + str(1) + '_save/'
    ...
    error_case.show_error_case(Y_predict, features, multilabel=is_multilabel,
                          if_print=False, index2label=index2label)
