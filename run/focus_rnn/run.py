...
def train(train_data_path, test_data_path, path_prefix, fold, load_model=False):
    ...
    for y in Y_train:
        for i, ii in enumerate(y):
            ...
            if ii == 1:
                Y_distribution[i] += 1
    print('Y_dis:', Y_distribution)
    #Y_distribution_r = {i: np.log(1.0 * (len(Y_train) - v) / v + 1) for i, v in Y_distribution.items()}
    Y_distribution_r = {i: np.sqrt(1.0 * (len(Y_train) - v) / max(1, v) + 1) for i, v in Y_distribution.items()}
    ...
    model = main_model.Model(W_embedding, Y_distribution_config, config, multilabel=is_multilabel)
    ...
    while timedelay < timedelay_num:
        ...
        if batches % checkpoint_num == 0:
            ...
            if best_loss > dev_loss or best_label < dev_label:
                ...
                if os.path.exists(root_path + '/all_in_one/demo/exported_models/' + mission + '/' + str(fold + 1) + '_save/'):
                    shutil.rmtree(root_path + '/all_in_one/demo/exported_models/' + mission + '/' + str(fold + 1) + '_save/')
