...
def rule_but_bei(logit_p, rule_logit_ind, w):
    '''
    :param logit_p
    :param rule_logiit_ind: [logit_rule, rule_ind]
    '''
    logit_rule = rule_logit_ind[0]
    rule_ind = rule_logit_ind[1]
    # print('logit_rule.size', logit_rule.size())
    # print('rule_ind.size', rule_ind.size())

    def calc_rule_constraints(logit_rule, rule_ind):
        # print('logit_rule', logit_rule.size())
        # print('logit_rule',rule_ind)
        rule_ind = torch.t(rule_ind.repeat(logit_rule.size()[1], 1))
        # print('rule_ind',rule_ind.size())

        distr = logit_rule * rule_ind * w
        return torch.exp(distr)
    distr = calc_rule_constraints(logit_rule, rule_ind)

    # print('logit_p.size', logit_p.size())
    # print('distr.size', distr.size())
    
    logit_q = logit_p * distr
